...
def getminHash(word, seed):
    ...
    for i in range(ord(unicode(u'\u052f'))-ord(unicode(u'\u0400'))):
        word_dict[26+i] = unichr(i+ ord(u'\u0400'))
    all_list = range(26+ord(u'\u052f')-ord(u'\u0400'))
    list = []
    hash_code = 0
        ...
        for block in block1:
            ...
            for i, id1 in enumerate(block1[block]):
                ...
                for j in range(i + 1, len(block1[block])):
                    id2 = block1[block][j]
                    if True or cluster_heads[id1][1] == cluster_heads[id2][1] and (cluster_heads[id1][2] != cluster_heads[id2][2] or "NIL" in cluster_heads[id1][2] or "NIL" in cluster_heads[id2][2]  ) :
                        if True or  "NIL" in cluster_heads[id1][2] or "NIL" in cluster_heads[id2][2] and cluster_heads[id1][1] == cluster_heads[id2][1]:
